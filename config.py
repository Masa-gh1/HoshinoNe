...
# キャッシュブロックサイズ[GB]
CACHE_BLOCK_SIZE_BYTES = BLOCK_SIZE * BLOCK_SIZE * DEFAULT_BLOCK_TYPE_BYTES # 最小のブロックサイズ (画像なら 縦x横x型)

# ブロックキャッシュページサイズ [ブロック数]
BLOCK_CACHE_PAGE_SIZE = 1024 # [ブロック数] 2^n であること
BLOCK_CACHE_PAGE_SIZE_BYTES = BLOCK_CACHE_PAGE_SIZE * CACHE_BLOCK_SIZE_BYTES

# キャッシュサイズ[GB]
MAX_CACHE_SIZE_GB = 8
MAX_CACHE_SIZE_BYTES = MAX_CACHE_SIZE_GB * 1024 * 1024 * 1024
MAX_CACHE_SIZE = MAX_CACHE_SIZE_BYTES // CACHE_BLOCK_SIZE_BYTES # [ブロック数]
...
def getDatetime(exif):
    ...
    for tag,sub in [("DateTime"         ,"SubSecTime"         ), #   306 0132h ファイル変更日時
        ...
        if tag in exif:
            ...
            result = dateTime.strftime("%Y-%m-%d %H:%M:%S.%f") if dateTime else None
    return result
...
HEADERS = [
    ...
    ("height"                     , "PixelYDimension"         ), # 40963 0101h 画像の高さ
    ("bits_per_sample"            , "BitsPerSample"           ), #   258 0102h 画像のビットの深さ
    ("datetime"                   , getDatetime               ), # ----- ----- 観測時刻
    ("exposure_time"              , "ExposureTime"            ), # 33434 829Ah 露出時間
    ("f_number"                   , "FNumber"                 ), # 33437 829Dh F ナンバー
    ("iso_speed"                  , getIsoSpeed               ), # ----- ----- 撮影感度
    ("focal_length"               , "FocalLength"             ), # 37386 920Ah レンズ焦点距離
    ("focal_plane_x_resolution"   , "FocalPlaneXResolution"   ), # 41486 A20Eh 焦点面の幅の解像度
    ("focal_plane_y_resolution"   , "FocalPlaneYResolution"   ), # 41487 A20Fh 焦点面の高さの解像度
    ("focal_plane_resolution_unit", "FocalPlaneResolutionUnit"), # 41488 A210h 焦点面解像度単位
]
HEADERS_EXIF = [
    #############################################################
    # 0th IFD TIFF Tag
